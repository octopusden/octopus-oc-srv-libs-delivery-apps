# controllers for checksums django models
...
class CheckSumsController(object):
    ...
    def add_location(self, file_r, location, loc_type, revision=None):
        ...
        # check we have such location
        _loc = models.Locations.objects.filter(path=location, loc_type=_loc_type, revision=revision, file_dst=None)
    ...
    def _delete_prev_locations(self, loc_r, reason=None):
        """
        Mark all previous file in the location specified as deleted.
        :param loc_r: location record from Model
        :param str reason: reason of deletion
        """
        # get a previous location with such a path/type/revision parameters and mark it 'deleted'

        _loc_set = models.Locations.objects.filter(
                loc_type=loc_r.loc_type, path=loc_r.path,
                input_date__lt=loc_r.input_date, revision=loc_r.revision).exclude(id=loc_r.id).order_by('input_date')

        if (_loc_set.count()) == 0:
            return

        return self.delete_location_cascade(_loc_set.last(), reason=reason)
    ...
    def delete_location_cascade(self, loc_r, reason=None):
        ...
        That means: file itself was deleted, and all previous files too.
        All previous locations will be deleted also.
        :param loc_r: location record from Model
        :param str reason: reason of deletion
        """
        if not loc_r:
        ...
        return self._delete_prev_locations(loc_r, reason=reason)
    ...
    def get_rn_gav(self, ci_type, version):
        ...
        if _citype.rn_artifactid:
            return _citype.get_rn_gav(version)
    ...
    def get_doc_gav(self, ci_type, version):
        """
        Returns GAV of ci_type documentation artifact
        :param str ci_type: CI type code, not None
        :return str: GAV (without packaging) of documentation artifact, or None if not found.
        """
        ci_type = ci_type.strip().upper()
