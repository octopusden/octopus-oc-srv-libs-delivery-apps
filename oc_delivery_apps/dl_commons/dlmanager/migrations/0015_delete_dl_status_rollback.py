...
def ff( apps, schema_editor ):
    ...
    obj_bs = BusinessStatus.objects.filter( description = dict_bs_deleted[ "deleted" ] ).last();
    ...
    for obj_dl_hist in DeliveryHistory.objects.filter( business_status = obj_bs ).all():
        ...
        qs_bs_hist = DeliveryHistory.objects.filter( id = obj_dl_hist.id ).exclude( business_status = obj_bs ).order_by( "-history_date" );
...
def bw( apps, schema_editor ):
    """ backward function """
    BusinessStatus = apps.get_model( "dlmanager", "BusinessStatus" );
    BusinessStatus.objects.get_or_create( description = dict_bs_deleted[ "deleted" ] );
